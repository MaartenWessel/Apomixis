...
file_path <- "C:/Users/Maarten/Documents/Plantstage/Seed set/Seed setGIZ001.csv"
df_raw <- read.csv(file_path, skip = 1, header = FALSE, stringsAsFactors = FALSE)

# Variable names
varnames <- c("Callus_Plant", paste0("M", 1:10), "Average", "Stdev", "Total")
...
# Axis labels: pretty wildtype
x_labels_all <- plants_all_sorted
x_labels_all[x_labels_all == "wildtype"] <- "Wild-type"

# Define bar colors: "tomato" for Wild-type, light blue for others
...
title("ToPAR - Seed set", font.main = 1) 
...
# Exemplary Figure: Only selected groups
exemplaryGroups <- c("wildtype", "C35P1", "C32P1", "C36P1", "C27P1", "C19P1", "C1P5", "C21P1", "C20P1")
plantsAll <- as.character(df10$Callus_Plant)
...
plants <- as.character(dfEx$Callus_Plant)
...
title("ToPAR - Seed set - overlap", font.main = 1) 
arrows(
  x0 = bar_centers_ex,
  y0 = means - stdevs,
  x1 = bar_centers_ex,
