...
# File paths
dataFile <- "C:/Users/Maarten/Documents/Plantstage/Amphasys/Datasheet39/CrossGatingFreq2Summary_with_second_sheet.xlsx"
wildtypeFile <- "C:/Users/Maarten/Documents/Plantstage/Amphasys/Wildtype.xlsx"
wildtypeGroupName <- "Wildtype"   # define wildtype 
wildtypeDisplayName <- "Wild-type" 
...
p2 <- ggplot(all_plot_df, aes(x = group, y = value, fill = group)) +
  ...
  theme_minimal() +
  labs(
    title = "AtTDM1-P17L - Pollen viability",
    x = NULL,
    y = "Pollen viability%"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 10, margin = margin(t = 8)),
