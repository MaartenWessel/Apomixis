...
# File paths 
dataFile <- "C:/Users/Maarten/Documents/Plantstage/Amphasys/Datasheet001/CrossGatingFreq2Summary_with_second_sheet.xlsx"
wildtypeFile <- "C:/Users/Maarten/Documents/Plantstage/Amphasys/Wildtype.xlsx"
wildtypeGroupName <- "Wildtype"   # Always define wildtype name here ONCE
...
data$Group <- paste0(data$Callus, data$Plant)
...
# Build display labels (Wild-type on x-axis)
...
# Remove count display label for x axis
displayLabels <- uniqueGroupsForPlot_display

# Statistical tests (Mann-Whitney U vs Wildtype) 
...
p2 <- ggplot(all_plot_df, aes(x = group, y = value, fill = group)) +
  ...
  theme_minimal() +
  labs(
    title = "ToPAR - Pollen viability",
    x = NULL,
    y = "Pollen viability%"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 6, margin = margin(t = 8)),
...
print(p2)
ggsave("topar_viability_general_trend.png", p2, width = 14, height = 6, dpi = 300)