...
file_path <- "C:/Users/Maarten/Documents/Plantstage/Seed set/Seed setGIZ39.csv"
df_raw <- read.csv(file_path, skip = 1, header = FALSE, stringsAsFactors = FALSE)

# Variable names (A to K = 11 columns: Construct_Plant, M1 to M10)
varnames <- c("Construct_Plant", paste0("M", 1:10))
...
title("AtTDM1-P17L - Seed set", font.main = 1) 
...
# Bar colors: "tomato" for Wild-type, light blue for others
...
title("AtTDM1-P17L - Seed set - overlap", font.main = 1) # font.main = 1 is plain (not bold)
arrows(
  x0 = bar_centers_ex,
  y0 = means - stdevs,
  x1 = bar_centers_ex,
